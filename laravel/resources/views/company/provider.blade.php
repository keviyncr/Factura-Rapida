...
<div class="panel panel-inverse" data-sortable-id="form-plugins-1">
    ...
    <div class="panel-heading">
        ...
        <div class="panel-heading-btn">
            <a href="#modal-addProvider"  data-toggle="modal" title="Nuevo proveedore" class="btn btn-md btn-icon btn-circle btn-danger"><i class="fa fa-plus"></i></a>
    ...
    <div class="panel-body ">
        <table id="data-table-provider" class="table table-striped table-bordered table-td-valign-middle">
            <thead>
                <tr>
                    <th width="1%">ID</th>
            ...
            <tbody>
                ...
                <tr class="gradeU">
                    <td width="1%" >{{ $provider->id }}</td>
                    <td>
                        <form method="post" action="{{ url ('providers/'.$provider->id)}}">
                            @csrf
                            @method('DELETE')
                            <div class="btn-group btn-group-justified">
                                <button type="button" title="Editar" class="btn btn-default" onclick="chargeEditModalProvider({{ $provider->id }});"><i class="fa fa-edit"></i></button>
                                <button type="submit" title="Borrar" class="btn btn-default" onclick="return confirm('Desea Borrar?');"><i class="fa fa-trash"></i></button>
                            </div>
                        </form> 
                    </td>
                    <td>{{ $provider->id_card }}</td>
                    <td>{{ $provider->type_idCard }}</td>
                    <td>{{ $provider->name_provider }}</td>
                    <td>{{ $provider->phone }}</td>
                    <td>{{ $provider->emails }}</td>
                </tr>
                @endforeach
...
<!-- #modal-addProvider -->
<div class="modal fade" id="modal-addProvider">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Agregar nuevo proveedore</h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            </div>
            <div class="modal-body">
                <form data-parsley-validate="true" action="{{ url('/providers') }}" method="POST" name="add_provider" id="add_provider" class="form-horizontal form-bordered">
                    @csrf
                    <!-- begin form-group -->
                    <div class="form-group row m-b-10">
                        <label class="col-lg-3 col-form-label">Nombre Proveedor: <span class="text-danger"></span></label>
                        <div class="col-lg-9 col-xl-9">
                            <input type="text" name="name_provider" data-parsley-required="true" class="form-control" placeholder="Nombre Proveedor" />
                        </div>                       
                    </div>
                    <!-- end form-group -->
            ...
            <div class="modal-footer">
                ...
                <button type="submit" form="add_provider" class="btn btn-success">Agregar</button>
...
<!-- #modal-addProvider -->
<!-- #modal-updateProvider -->
<div class="modal fade" id="modal-updateProvider">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <form data-parsley-validate="true" action="{{ url('/providers') }}" method="POST" name="update_provider" id="update_provider" class="form-horizontal form-bordered">
                    @csrf
                    @method('PATCH')
                    <!-- begin form-group -->
                    <div class="form-group row m-b-10">
                        <label class="col-lg-3 col-form-label">Nombre Proveedor: <span class="text-danger"></span></label>
                        <div class="col-lg-9 col-xl-9">
                            <input type="text" name="name_provider" id="name_providerU" data-parsley-required="true" class="form-control" placeholder="Nombre Proveedor" />
                            ...
                            <div class="row row-space-6">
                                <div class="col-4">
                                    <select class="form-control" data-parsley-required="true" name="id_province" id="id_provinceU" data-parsley-group="step-2">                                                               
                                        <option style="color: black;" value="0" disabled="true">Provincia</option>
                                        @foreach($provinces as $province)
                                        <option style="color: black;" value="{{ $province->id }}">{{ $province->province }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-4">
                                    <select class="form-control" data-parsley-required="true" name="id_canton" id="id_cantonU" data-parsley-group="step-2">
                                        <option style="color: black;" value="0"  disabled="true">Cantón</option>

                                    </select>
                                </div>
                                <div class="col-4">
                                    <select class="form-control" data-parsley-required="true" name="id_district" id="id_districtU" data-parsley-group="step-2">
                                        <option style="color: black;" value="0"  disabled="true">Distrito</option>
            ...
            <div class="modal-footer">
                <a href="javascript:;" class="btn btn-white" data-dismiss="modal">Cerrar</a>
                <button type="submit" form="update_provider" class="btn btn-success">Actualizar</button>
